...
const App = () => {
  ...
  const updatedImagesList = imagesList.filter(each => (
    each.category === activeTabId
  ))

  const activeImage = imagesList.find(each => (
    each.id === activeImageId
  ))

  const changeTabId = tabId => {
    setActiveTabId(tabId)
  }

  const onClickThumbnailImage = id => {
    const index = Math.ceil(Math.random() * imagesList.length)

    if (activeImageId === id){
      setScore(score + 1)
      setActiveImageId(imagesList[index].id)
    }else{
      clearInterval(timerID.current)
      setIsGameOver(true)
    }
...
const playAgain = () => {
  ...
  setActiveImageId(imagesList[0].id)
  setScore(0)
  setIsGameOver(false)
  setTimeInSeconds(60)
  startTimer()
...
const gameOverView = () => (
  <div className="game-over-container">
    <h1 className="game-over-heading">GameOver</h1>
    <h1 className="your-score">Your Score</h1>
    <h1 className="score">{score}</h1>
    <button className="button" type="button" onClick={playAgain}>Play Again</button>
  </div>
)
  ...
  return (
    ...
    <nav className="nav-container">
      ...
      <div className="score-board">
        ...
        <p className="seconds">{timeInSeconds} secs</p>
      </div>
    </nav>
    ...
    <div className="main-container">
      ...
      <div className="top-image-container">
        <img src={activeImage.imageUrl} className="active-image" alt="active-image"/ >
      </div>

      <ul className="tabs-list-container">
        {tabsList.map(each=> (
          <Faqs tabItem={each} key={each.tabId} onClickTabBtn={changeTabId} isActive={each.tabId === activeTabId} />
        ))}
      </ul>

      <ul className="images-list-container">
        {updatedImagesList.map(each => (
          <FaqItem imageDetails={each} key={each.id} clickThumbnailImage={onClickThumbnailImage}/>
        ))}
      </ul>
</div>
    }
